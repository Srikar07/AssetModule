...
import { DealsTable } from '../../../../../../modules/asset/_fake/fake-server/deals.table';
    ...
    if (e.target.value.length > 0) {
      ...
      setTimeout(() => {
        ...
        // this.data = this.searchInData(e.target.value);
          ...
          //var filterDocs = documents.items.filter(item => { return item.description.includes(e.target.value) })
          //var filteredList = documents;
          //documents.items = filterDocs;


          var filterdealItems = deals.items.filter(item => { return item.name.includes(e.target.value) })
          var filteredDeals  = {
              title: 'Deals',
              type: 3,
              items: filterdealItems
          };

          console.log(filteredDeals);


          this.data = [filteredDeals];
        this.loading = false;
